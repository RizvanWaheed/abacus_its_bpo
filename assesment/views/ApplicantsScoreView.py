...
from aeams.models import TblUsers as Users, TblDomains as Domains, TblSetups as Setups, ApplicantSchedulers, TblSetupSetups as setupSetups
...
def index(request, page=None):
	...
	if request.session.has_key('is_logged'):
		...
		#
		return render(request, 'assesment/index_angularjs.html',
...
def getApplicantOrientationScore(request):
    ...
    fc = Q(modified__gte=fm_date, modified__lte=to_date, sub_state_id=sub_state_id)
    ...
    applicant = Applicants.objects.filter(fc).values('id', 'first_name', 'last_name', 'project__name', 'joining_date')
    ...
    if applicant_list:
        ...
        for index, item in enumerate(applicant_list):
            recruiter = roleOrientaion(item.get('id'), 'head')

            if not recruiter:
                recruiter = roleOrientaion(item.get('id'), 'manager')

            if not recruiter:
                recruiter = roleOrientaion(item.get('id'), 'officer')

            operations = roleOrientaion(item.get('id'), 'op_manager')

            if not operations:
                operations = roleOrientaion(item.get('id'), 'op_lead')

            trainer = roleOrientaion(item.get('id'), 'trainer')

            # ApplicantOrientation.objects.filter(applicant_id=item.get('id'), role='trainer',
            #                                               status=1).values(*tuple(select_args));

            recruiter_list = list(recruiter)
            operations_list = list(operations)
            trainer_list = list(trainer)

            if recruiter_list:
                applicant_list[index]['recruiter'] = roleOrientaionPercentage(orientaionCalculation(recruiter_list))
            if operations_list:
                applicant_list[index]['operations'] = roleOrientaionPercentage(orientaionCalculation(operations_list))
            if trainer_list:
                applicant_list[index]['trainer'] = roleOrientaionPercentage(orientaionCalculation(trainer_list))

            # if recruiter_dict['total'].has_key(attributes)


    return JsonResponse({'applicant': applicant_list, 'success_status': True}, safe=False)

def roleOrientaionPercentage(roleOrientaion):
    role_dict = dict()
    # print(roleOrientaion)
    for key, value in roleOrientaion.items():
        if value['score'] <= 0:
            role_dict[key] = {
                'attributes' : value['attributes'],
                'scorable': value['scorable'],
                'deductable': value['deductable'],
                'score': value['score'],
                'percentage': 0
            }
        else:
            percentage = round((value['score']/value['scorable'])*100, 2)
            role_dict[key] = {
                'attributes': value['attributes'],
                'scorable': value['scorable'],
                'deductable': value['deductable'],
        ...
        # print(value['scorable'])
        # print(value['deductable'])
        # print(key)
        # // if(item.get(scor))
    return role_dict

def roleOrientaion(applicant_id, role_name):
    select_args = ['id', 'orientation_id', 'orientation__name', 'orientation__parent_id', 'applicant_id', 'active',
                   'checked', 'status',
                   'orientation_level', 'role', 'orientation__parent__name']

    # recruiter = ApplicantOrientation.objects.filter(applicant_id=applicant_id, role=role_name,
    #                                                 status=1).values('id', 'orientation_id', 'orientation__name',
    #                                                                  'orientation__parent_id', 'applicant_id', 'active',
    #                                                                  'checked', 'status',
    #                                                                  'orientation_level', 'role',
    #                                                                  'orientation__parent__name');

    return ApplicantOrientation.objects.filter(applicant_id=applicant_id, role=role_name,
                                               status=1).values(*tuple(select_args));

def orientaionCalculation(recruiter_list):
    recruiter_dict = dict()
    recruiter_dict['total'] = {
        'attributes': 0, 'scorable': 0, 'deductable': 0, 'score': 0
    }
    for index, item in enumerate(recruiter_list):
        # print(item)
        # if item['orientation__parent_id'] == 2500:
        recruiter_dict['total']['attributes'] = recruiter_dict['total']['attributes'] + 1
        recruiter_dict['total']['scorable'] = recruiter_dict['total']['scorable'] + 1
        recruiter_dict['total']['deductable'] = recruiter_dict['total']['deductable'] + 1

        parent_name = item['orientation__parent__name'].replace(" ", "_")
        if not parent_name in recruiter_dict:
            recruiter_dict[parent_name] = {
                'attributes': 0, 'scorable': 0, 'deductable': 0, 'score': 0
            }

        recruiter_dict[parent_name]['attributes'] = \
            recruiter_dict[parent_name][
                'attributes'] + 1

        if int(item['orientation_level']) > 0:
            recruiter_dict[parent_name]['scorable'] = \
                recruiter_dict[parent_name][
                    'scorable'] + 1
        elif int(item['orientation_level']) < 0:
            recruiter_dict[parent_name]['deductable'] = \
                recruiter_dict[parent_name][
                    'deductable'] + 1

        if item['checked'] == 1:
            recruiter_dict['total']['score'] = \
                recruiter_dict['total'][
                    'score'] + int(item['orientation_level'])

            recruiter_dict[parent_name]['score'] = \
                recruiter_dict[parent_name][
                    'score'] + int(item['orientation_level'])
        ...
        # if not 'attributes' in recruiter_dict[item['orientation__parent__name']]:
        #     recruiter_dict[item['orientation__parent__name']]['attributes'] = 0
        # if not 'scorable' in recruiter_dict[item['orientation__parent__name']]:
        #     recruiter_dict[item['orientation__parent__name']]['scorable'] = 0
        # if not 'deductable' in recruiter_dict[item['orientation__parent__name']]:
        #     recruiter_dict[item['orientation__parent__name']]['deductable'] = 0
        # if not 'score' in recruiter_dict[item['orientation__parent__name']]:
        #     recruiter_dict[item['orientation__parent__name']]['score'] = 0

        # if not 'attributes' in recruiter_dict['total']:
        #     recruiter_dict['total']['attributes'] = 0
        # if not 'scorable' in recruiter_dict['total']:
        #     recruiter_dict['total']['scorable'] = 0
        # if not 'deductable' in recruiter_dict['total']:
        #     recruiter_dict['total']['deductable'] = 0
        # if not 'score' in recruiter_dict['total']:
        #     recruiter_dict['total']['score'] = 0
...
def searchTodayApplicantList(request):
    ...
    if desi != 'officer':
        ...
        applicant = Applicants.objects.filter(modified__gte=fm_date, state_name=state).values('id', 'cnic', 'gender',
...
def getApplicants(request):
    ...
    applicant = Applicants.objects.filter(fc).values(*tuple(select_args))
    print(applicant.query)
    applicant_list = list(applicant)
    # print(applicant_list)
...
def setApplicantAssesment(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if (ApplicantOrientation.objects.filter(applicant_id=var_applcant.id)):
