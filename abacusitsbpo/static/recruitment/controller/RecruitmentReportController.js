RecruitmentModule.controller('RecruitmentReportController', ['$scope', '$location', '$upload', '$filter', 'Svc',
	function($scope,$location,$upload,$filter,Svc){
		...
		$scope.downloadApplicant = function(){
			...
			console.log($scope.applicantInformation);
			 ...
			 ($('<form/>', {
                'id':       'tmpPDFForm',
                'action':   "download_applicant",
                'method':   'get',
                'target': '_blank'
            }).append($('<input />', { 'type': 'hidden', 'name': 'officer',  'value': $scope.applicantInformation.officer })
            ).append($('<input />', {  'type': 'hidden', 'name': 'status',    'value': $scope.applicantInformation.sub_state_id })
