SetupsModule.controller('BasicOptionsController', ['$scope', '$location', '$upload', '$filter','Svc',
	function($scope, $location, $upload, $filter, Svc){
		//return;
        //$scope.setLoadingTemplate(false);
		$scope.showTabsInView = false;
		$scope.activeTabIndex = 'country';
		...
		$scope.domined = domains;
		console.log($scope.domined);
		$scope.varBasicOptionName = 'country';
		$scope.loadBasicOptionsList = function(atab){
			$scope.varBasicOptionName = atab;

			console.log(atab)
			console.log($scope.activeTabIndex)

		};
		$scope.closeBasicOptionModel = function() {
			$('#basicOptionsModal').modal('hide')
		};
		$scope.openBasicOptionModel = function(key) {
			$scope.domain = {
				domain_type:$scope.varBasicOptionName,
				name:'',
				code:''
			}

			$('#basicOptionsModal').modal('show');
		};
		$scope.saveBasicOption = function(domain){

			// $scope.scheduler.schedule_on = moment($scope.scheduler.schedule_on).format('yyyy-MM-DD');
			Svc.save(
				AppCore.parseNGAjaxReq({action: 'set_basic_options'}),
			    JSON.stringify($scope.domain),
				function(response) {
					// console.log(response.success_status)
					// console.log(response.data.success_status)
